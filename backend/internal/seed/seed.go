// Package seed loads demo data and the platform super admin.
// Idempotent: safe to run repeatedly. All seeded accounts share
...
// RunAdminOnly ensures just the platform super admin exists — the safe
// automatic mode for production first boots.
func RunAdminOnly(ctx context.Context, db *pgxpool.Pool, logger *slog.Logger) error {
	hash, err := seedHash()
	if err != nil {
		return err
	}
	now := time.Now()
	_, err = ensureUser(ctx, postgres.NewUserRepo(db), &auth.User{
...
// Run seeds the super admin plus the full "Teresa's Eatery" demo
// tenant: role accounts, menu, inventory, employees, and customers.
func Run(ctx context.Context, db *pgxpool.Pool, logger *slog.Logger) error {
	...
	if err := RunAdminOnly(ctx, db, logger); err != nil {
		return err
	}
