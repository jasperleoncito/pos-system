...
- **Billing (C5):** Xendit-invoice subscriptions — monthly/yearly, prices in `platform_settings` (centavos, super-admin editable), one `subscriptions` row per tenant (pending|active|inactive) + `subscription_payments` ledger. ALL period math in SQL (Postgres interval clamping, never Go AddDate); webhook `POST /webhooks/xendit` verified by `x-callback-token`, idempotent via conditional UPDATE. `middleware.RequireActiveSubscription` → 402 on tenant routes (fail-open, super-admin bypass, 60s Redis cache `billing:sub:{tenant}`); notifications + GET /tenant/settings stay open for the pay-modal/blocked-screen. Hourly worker sweep sends 3-day notices + deactivates overdue. `tenants.status` suspend stays separate. Tracking: `docs/BILLING-PLAN.md`.
